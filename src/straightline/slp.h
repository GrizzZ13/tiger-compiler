...
public:
  ...
  //        and ` IntAndTable *Interp(Table *)`
  virtual int MaxArgs() const = 0;
  virtual int ExpNumber() const = 0;
  virtual IntAndTable *Interp(Table *table, std::list<int> &list) const = 0;
};
...
public:
  ...
  // TODO: you'll have to add some definitions here (lab1).
  int MaxArgs() const override;
  int ExpNumber() const override;
  IntAndTable *Interp(Table *table, std::list<int> &list) const override;
